...
private void OnCollisionEnter(Collision collision)
{
    ...
    // Check if balloon hit from the front (dot > 0 means balloon is in front of target)
    Vector3 directionToballoon = (transform.position - target.position).normalized;
    bool hitFromFront = Vector3.Dot(target.forward, directionToballoon) > 0f;
    ...
    if (distance <= 5f && !collision.gameObject.CompareTag("Arrow"))
    {
        if (!hitFromFront)
        {
            ...
            BalloonManager.Instance.ReportBalloonDown();
            return;
        }
        Explode();
